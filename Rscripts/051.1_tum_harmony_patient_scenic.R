# Continued from 050_ and pySCENIC output
# hvg in scanpy with batch_key="dataset"
# 2026-04-07

# Settings ----
## Make directories
wd <- getwd()
analysis_step <- "051.1_tum_harmony_patient_scenic"
...
# load data ----
seu <- readRDS("RDSfiles/seu_050.1_tum_harmony_patient.RDS")
auc_mtx <- read_tsv(file = "adata/tum_harmony_patient/auc_mtx_hvg_per_dataset.txt") %>% 
...
ggsave("cluster_scenic.pdf", path = plot_path, width = 45, height = 30, units = "mm")
...
DimPlot(seu, cols = "polychrome", group.by = "RNA_snn_res.0.2", raster = TRUE, raster.dpi = c(600, 600), pt.size = 2) &
  theme_panel() & NoAxes() & labs(title = "harmony_clustering") &
  guides(color = guide_legend(override.aes = list(size = 3, alpha = 1), ncol = 3))
ggsave("harmony_cluster_on_scenic_UMAP.pdf", path = plot_path, width = 40, height = 30, units = "mm")
...
saveRDS(seu, "RDSfiles/seu_051.1_tum_harmony_patient_auc_mtx.RDS")
