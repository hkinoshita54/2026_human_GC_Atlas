...
analysis_step <- "310.1_cellchat_MP14_resid"
...
## filter by ccgroup
ccgroup_levels <- c(unique(tum$MP14_resid_q), levels(fib$celltype2), levels(ec$celltype2), levels(myo$celltype2), levels(mye$celltype2))
...
saveRDS(cellchat_list, "RDSfiles/cellchat_list_310.1_MP14_resid.rds")
...
# MP14resid_high as target ----
mp14_target_compare <- lr_summary %>%
  filter(target %in% c("MP14_4", "MP14_1_2")) %>%
  select(
  ...
  mutate(
    delta_fraction_significant =
      fraction_significant_MP14_4 - fraction_significant_MP14_1_2,
    delta_median_prob_among_testable =
      median_prob_among_testable_MP14_4 - median_prob_among_testable_MP14_1_2,
    delta_median_prob_among_significant =
      median_prob_among_significant_MP14_4 - median_prob_among_significant_MP14_1_2
  ) %>%
  arrange(
...
openxlsx2::write_xlsx(mp14_target_compare, file.path(res_path, "MP14_4_vs_MP14_1_2_target_comparison.xlsx"))

# MP14resid_high as source ----
mp14_source_compare <- lr_summary %>%
  filter(source %in% c("MP14_4", "MP14_1_2")) %>%
  select(
    source, target, ligand, receptor, pathway_name, interaction_name,
    n_testable_samples, n_significant_samples, fraction_significant,
    median_prob_among_testable, median_prob_among_significant
...
openxlsx2::write_xlsx(mp14_source_compare, file.path(res_path, "MP14_4_vs_MP14_1_2_source_comparison.xlsx"))

# correlation with sequencing depth ----
library(ggpubr)
Idents(seu_all) <- "ccgroup"
FeatureScatter(seu_all, "MP14_resid", "nFeature_RNA", pt.size = 1) & 
  stat_cor(method = "spearman") &
  theme_panel() &
  theme(plot.title = element_blank(), axis.title.y = element_text(angle = 90), axis.text.x = element_text(angle = 90)) & NoLegend()
ggsave("fscatter_MP14resid_nFeature.pdf", path = plot_path, width = 40, height = 30, units = "mm", device = cairo_pdf)

VlnPlot(seu_all, features = "nFeature_RNA", group.by = "MP14_resid_state", pt.size = 0)  &
...
ggsave("vln_MP14resid_nFeature.pdf", path = plot_path, width = 30, height = 35, units = "mm", device = cairo_pdf)
