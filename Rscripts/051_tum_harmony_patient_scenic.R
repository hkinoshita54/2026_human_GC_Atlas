...
## Make directories
wd <- getwd()
analysis_step <- "051_tum_harmony_patient_scenic"
...
auc_mtx <- read_tsv(file = "adata/tum_harmony_patient/auc_mtx.txt") %>% 
...
DimPlot(seu, cols = "polychrome", group.by = "RNA_snn_res.0.2", raster = TRUE, raster.dpi = c(600, 600), pt.size = 2) &
  ...
  guides(color = guide_legend(override.aes = list(size = 3, alpha = 1), ncol = 3))
ggsave("harmony_clustering.pdf", path = plot_path, width = 40, height = 30, units = "mm")
