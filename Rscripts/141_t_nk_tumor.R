...
seu <- readRDS("../2025_Kang/RDSfiles/seu_060_t_nk.RDS")
...
seu <- readRDS("../2025_Kumar_2/RDSfiles/seu_060_t_nk.RDS")
seu <- harmonize_symbols(seu)
str(seu[[]])
seu$seurat_clusters <- NULL
dataset <- seu$dataset %>% unique() %>% as.character()
...
seu <- readRDS("../2023_Jeong/RDSfiles/seu_050_t_nk.RDS")
seu <- harmonize_symbols(seu)
str(seu[[]])
seu$seurat_clusters <- NULL
seu$Lauren <- "Diffuse"
...
seu <- readRDS("../2026_Jiang_GSE163558/RDSfiles/seu_050_t_nk.RDS")
seu <- harmonize_symbols(seu)
str(seu[[]])
seu$seurat_clusters <- NULL
dataset <- seu$dataset %>% unique() %>% as.character()
...
## Kim et al. GSE150290
seu <- readRDS("../2026_Kim/RDSfiles/seu_050_t_nk.RDS")
seu <- harmonize_symbols(seu)
str(seu[[]])
seu$seurat_clusters <- NULL
dataset <- seu$dataset %>% unique() %>% as.character()
...
seu <- readRDS("../2026_Sun_OMIX001073/RDSfiles/seu_050_t_nk.RDS")
seu <- harmonize_symbols(seu)
str(seu[[]])
seu$seurat_clusters <- NULL
dataset <- seu$dataset %>% unique() %>% as.character()
...
seu <- readRDS("../2026_Zhao_OMIX013242/RDSfiles/seu_050_t_nk.RDS")
seu <- harmonize_symbols(seu)
str(seu[[]])
seu$seurat_clusters <- NULL
dataset <- seu$dataset %>% unique() %>% as.character()
...
## Gao et al. GSE270680
seu <- readRDS("../2026_Gao_GSE270680/RDSfiles/seu_050_t_nk.RDS")
seu <- harmonize_symbols(seu)
str(seu[[]])
dataset <- seu$dataset %>% unique() %>% as.character()
seu_list[[dataset]] <- seu
...
seu <- readRDS("../2026_Tsutsumi_GSE201347/RDSfiles/seu_050_t_nk.RDS")
seu <- harmonize_symbols(seu)
str(seu[[]])
seu$seurat_clusters <- NULL
dataset <- seu$dataset %>% unique() %>% as.character()
...
keep <- Matrix::rowSums(seu[["RNA"]]$counts > 0) >= 30
...
ggsave("cluster_res0.5.pdf", path = plot_path, width = 40, height = 30, units = "mm")
...
# 2nd clustering ----
seu1 <- seu
seu <- subset(seu1, subset = seurat_clusters %in% c(5,11,13), invert = TRUE) # remove low quality cluster (ambient RNA contamination)
...
DimPlot(seu, cols = "polychrome", raster = TRUE, raster.dpi = c(600, 600), pt.size = 2) &
  theme_panel() & NoAxes() & labs(title = "seurat_clusters") &
  guides(color = guide_legend(override.aes = list(size = 3, alpha = 1), ncol = 3))
ggsave("cluster_2_res0.5.pdf", path = plot_path, width = 40, height = 30, units = "mm")
...
VlnPlot(seu, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3, pt.size = 0) 
ggsave("QC_vln_2_res0.5.png", path = plot_path, width = 15, height = 3, units = "in", dpi = 150)
...
markers %>%
  ...
  slice_head(n = 20) %>%
  ungroup() -> top20
write_csv(top20, file = file.path(res_path, "top20_markers_2_res0.5.csv"))

# Add celltype2 annotation ----
seu2 <- seu
seu <- subset(seu2, subset = seurat_clusters %in% c(14), invert = TRUE) # remove low quality cluster (ambient RNA contamination)

seu$celltype2 <- NA_character_
seu$celltype2[seu$seurat_clusters %in% c(0)]  <- "CD8_GZMK"
seu$celltype2[seu$seurat_clusters %in% c(1)]  <- "Treg"
seu$celltype2[seu$seurat_clusters %in% c(2)]  <- "CD8_TRM"
seu$celltype2[seu$seurat_clusters %in% c(3)]  <- "Naive_TCM"
seu$celltype2[seu$seurat_clusters %in% c(4)]  <- "CD4_IL7R"
seu$celltype2[seu$seurat_clusters %in% c(5)]  <- "CD8_Exhausted"
seu$celltype2[seu$seurat_clusters %in% c(6)]  <- "NK"
seu$celltype2[seu$seurat_clusters %in% c(7)]  <- "CD4_CXCL13"
seu$celltype2[seu$seurat_clusters %in% c(8)]  <- "Th17_Tc17"
seu$celltype2[seu$seurat_clusters %in% c(9)]  <- "gdT_NKlike"
seu$celltype2[seu$seurat_clusters %in% c(10)] <- "Stress_TNK"
seu$celltype2[seu$seurat_clusters %in% c(11)] <- "MAIT"
seu$celltype2[seu$seurat_clusters %in% c(12)] <- "Cycling_TNK"
seu$celltype2[seu$seurat_clusters %in% c(13)] <- "IFN_TNK"
seu$celltype2[seu$seurat_clusters %in% c(14)] <- "Unclear_TNK"
...
tnk_markers <- c(
  "CCR7", "TCF7",        # Naive_TCM
  "IL7R", "CD40LG",     # CD4_IL7R
  "CXCL13", "MAF",      # CD4_CXCL13
  "FOXP3", "IL2RA",     # Treg
  "GZMK", "CD8A",       # CD8_GZMK
  "ITGA1", "ZNF683",    # CD8_TRM
  "LAG3", "ITGAE",      # CD8_Exhausted
  "FCGR3A", "FGFBP2",   # NK
  "TRDC", "KLRD1",      # gdT_NKlike
  "TRAV1-2", "SLC4A10", # MAIT
  "IL17A", "CCR6",      # Th17_Tc17
  "MKI67", "TOP2A",     # Cycling_TNK
  "MX1", "ISG15",       # IFN_TNK
  "FOS", "JUN"          # Stress_TNK
)
sapply(tnk_markers, save_fp, seu, fp_path_2)

DotPlot(seu, group.by = "celltype2", features = tnk_markers, dot.scale = 2.3) + 
...
ggsave("dotplot.pdf", path = plot_path, width = 100, height = 45, units = "mm")
...
saveRDS(seu, file = "RDSfiles/seu_141_t_nk_tumor.RDS")


seu3 <- seu[, !is.na(seu$annotation2)]
DimPlot(seu3, group.by = "annotation2", cols = "polychrome", raster = TRUE, raster.dpi = c(600, 600), pt.size = 2) &
  theme_panel() & NoAxes() &
  guides(color = guide_legend(override.aes = list(size = 3, alpha = 1), ncol = 2))
