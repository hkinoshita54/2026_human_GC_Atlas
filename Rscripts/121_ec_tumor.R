...
seu <- subset(seu, subset = celltype1 %in% c("EC") & tissue_type == "T")
...
ggsave("cluster_res0.2.pdf", path = plot_path, width = 45, height = 30, units = "mm")
...
add_feat <- c("PECAM1", "EMCN", "SOST", "PLAT", "CLEC1B", "CLEC4G", "LYVE1", "PROX1", "SEMA3G", "ACKR1", "RGCC", "PDGFRB", "RGS5", "DCN", "ACTA2")
...
ggsave("QC_vln_res0.2.png", path = plot_path, width = 15, height = 3, units = "in", dpi = 150)

# 2nd clustering ----
seu1 <- seu
seu <- subset(seu1, subset = seurat_clusters %in% c(0, 6), invert = TRUE) # remove low quality cluster (low nFeature_RNA) and pericyte carry over
...
seu$celltype2[seu$seurat_clusters %in% c(2)] <- "ArtEC" 
seu$celltype2[seu$seurat_clusters %in% c(0,5)] <- "VenEC" 
seu$celltype2[seu$seurat_clusters %in% c(1,3,4)] <- "CapEC" 
seu$celltype2[seu$seurat_clusters %in% c(6)] <- "LEC" 
seu$celltype2 <- factor(seu$celltype2, levels = c("ArtEC", "VenEC", "CapEC", "LEC"))
...
saveRDS(seu, file = "RDSfiles/seu_121_ec_tumor.RDS")
