...
## filter by ccgroup
ccgroup_levels <- c(unique(tum$MP14_state), levels(fib$celltype2), levels(ec$celltype2), levels(myo$celltype2), levels(mye$celltype2))
...
saveRDS(cellchat_list, "RDSfiles/cellchat_list_310_MP14.rds")
...
mp14_target_compare <- lr_summary %>%
  filter(target %in% c("MP14_4th", "MP14_1st2nd")) %>%
  select(
  ...
  mutate(
    delta_fraction_significant =
      fraction_significant_MP14_4th - fraction_significant_MP14_1st2nd,
    delta_median_prob_among_testable =
      median_prob_among_testable_MP14_4th - median_prob_among_testable_MP14_1st2nd,
    delta_median_prob_among_significant =
      median_prob_among_significant_MP14_4th - median_prob_among_significant_MP14_1st2nd
  ) %>%
  arrange(
...
openxlsx2::write_xlsx(
  mp14_target_compare,
  file.path(res_path, "MP14_4th_vs_MP14_1st2nd_target_comparison.xlsx")
)


library(ggpubr)
Idents(seu_all) <- "ccgroup"
FeatureScatter(seu_all, "MP_14_UCell", "nFeature_RNA", pt.size = 1) & 
  stat_cor(method = "spearman") &
  theme_panel() &
  theme(plot.title = element_blank(), axis.title.y = element_text(angle = 90), axis.text.x = element_text(angle = 90)) & NoLegend()
ggsave("fscatter_MP14UCell_nFeature.pdf", path = plot_path, width = 40, height = 30, units = "mm", device = cairo_pdf)

VlnPlot(seu_all, features = "nFeature_RNA", group.by = "MP14_state", pt.size = 0)  &
...
ggsave("vln_MP14UCell_nFeature.pdf", path = plot_path, width = 30, height = 35, units = "mm", device = cairo_pdf)
