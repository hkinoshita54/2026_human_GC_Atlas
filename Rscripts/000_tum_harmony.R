...
seu <- readRDS("../2025_Kang/RDSfiles/seu_100_tum.RDS")
str(seu[[]])
...
seu <- readRDS("../2023_Jeong/RDSfiles/seu_100_tum.RDS")
str(seu[[]])
seu$celltype1 <- NULL
seu$celltype2 <- NULL
...
seu <- readRDS("../2026_Jiang_GSE163558/RDSfiles/seu_100_tum.RDS")
str(seu[[]])
...
# ## Gao et al. GSE270680
# seu <- readRDS("../2026_Gao_GSE270680/data/Tertiary lymphoid structures at varied stages exhi/SC/sc.rds")
# seu <- subset(seu, majorCluster == "Epithelium")
# seu <- subset(seu, loc %in% c("T", "N"))
# seu <- harmonize_symbols(seu)
# str(seu[[]])
# seu@meta.data <- seu@meta.data %>% dplyr::select(
#   orig.ident = library,
#   nCount_RNA,
#   nFeature_RNA,
#   sample = library,
