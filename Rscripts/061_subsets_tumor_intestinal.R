...
## Make directories
analysis_step <- "061_subsets_tumor_intestinal"
...
seu <- subset(seu_all, subset = celltype2 == "Intestinal")

## filter out patient with less than 10 cells
patient_counts <- table(seu$patient)
patients_keep <- names(patient_counts[patient_counts >= 10])
